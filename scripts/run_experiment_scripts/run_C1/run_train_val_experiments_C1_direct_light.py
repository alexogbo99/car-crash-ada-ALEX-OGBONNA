...
METRICS_CSV = RESULTS_DIR / "train_val_metrics_direct.csv"
TUNING_CSV = RESULTS_DIR / "tuning_log_direct.csv"
...
def prepare_xy_subsampled(df, features, target, fraction=0.05):
    if len(df) > 10000:
...
def main():
    print("=== STARTING EXPERIMENT: C1 (Crash Counts) | DIRECT | LIGHT ===")
    ...
    for target in TARGETS:
        ...
        for tier_name in TIER_LIST:
            ...
            for hist_name in HISTORY_LIST:
                ...
                # Subsample
                X_train, y_train = prepare_xy_subsampled(train_df, full_feats, target, fraction=0.05)
                X_val, y_val     = prepare_xy_subsampled(val_df, full_feats, target, fraction=0.20)
                
                for algo in ALGOS:
                    print(f"    > ALGO: {algo} ... ", end="")
                    t0 = time.time()
                    ...
                    try:
                        ...
                        if needs_scaling:
                            ...
                            with open(MODELS_DIR / f"scaler_{algo}_{tier_name}_{target}.pkl", "wb") as f:
                        ...
                        # Train
                        model = get_optimized_model(algo, input_dim=X_train_in.shape[1], tuning_csv=TUNING_CSV, is_search=True)
                        ...
                        rpath = RESULTS_DIR / "train_val_results_C1_Direct.json"
