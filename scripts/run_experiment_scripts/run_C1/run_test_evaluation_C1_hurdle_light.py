...
RESULTS_DIR = ROOT / "results" / "C1_Hurdle_Light"
...
def load_or_fit_train_scaler(
    ...
    *,
    ...
    """
    Returns a StandardScaler fit on TRAIN data only.
    - Caches to scaler_path so you fit once per (model/tier/target/algo).
    - Uses streaming partial_fit via pyarrow when available.
    """
...
# ==============================================================================
def main():
    print("=== FINAL TEST EVALUATION (C1 | Hurdle | Light | DUAL METRIC) ===")
    with joblib.parallel_backend('threading', n_jobs=-1):
        
        # 1. Load Result Table
        results_file = RESULTS_DIR / "train_val_results_C1_Hurdle_light.json"
        
        if not results_file.exists():
            # Fallback
            results_file = RESULTS_DIR / "train_val_results.json"
            
        if not results_file.exists(): 
             print(f"No results found at: {RESULTS_DIR}"); return
        ...
        for sort_metric, out_filename in METRIC_CONFIGS:
            print(f"\n\nSTARTING PASS: Selecting Regressors by {sort_metric.upper()}...")
            ...
            for target in TARGETS:
                ...
                for tier in TIER_LIST:
                    try:
                        # 1. Select Best Regressor (Sorted by Current Metric)
                        ...
                        # 2. Load Gatekeeper
                        gk_path = RESULTS_DIR / f"champion_gatekeeper_{tier}_{target}.pkl"
                        if not gk_path.exists(): 
                            print(f"Missing Gatekeeper: {gk_path}")
                            continue
                        
                        with open(gk_path, "rb") as f: gatekeeper = pickle.load(f)
                        print(f"{tier}: Gatekeeper + Regressor({best_algo}) [Val {sort_metric}: {best_row[sort_metric]:.4f}]")
                        ...
                        # 4. Predict Gatekeeper
                        prob_crash = gatekeeper.predict_proba(X_test)[:, 1]

                        # 5. Load & Predict Regressor (FIT ON TRAIN ONCE, CACHE)
                        model_name = f"regressor_hurdle_{best_algo}_{tier}_{target}"

                        if best_algo in ["GLM", "MLP", "GRU"]:
                            ...
                            X_test_reg = scaler.transform(X_test).astype(np.float32)
                        else:
                            X_test_reg = X_test
                        ...
                        if best_algo in ["MLP", "GRU"]:
                            ...
                            y_count = predict_with_torch(model, X_test_reg, is_gru=(best_algo=="GRU"))
                        else:
                            ...
                            print_model_details(best_algo, model)
                            y_count = model.predict(X_test_reg)

                        # 6. Combine
                        final_pred = prob_crash * y_count
                        ...
                        del model, gatekeeper, X_test, y_test; gc.collect()

                    except Exception as e: 
                        print(f"Error {tier}: {e}")
                        traceback.print_exc()

            with open(RESULTS_DIR / out_filename, "w") as f: 
