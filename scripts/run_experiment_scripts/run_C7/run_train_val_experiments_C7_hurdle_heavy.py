# scripts/run_experiment_scripts/run_C7/run_train_val_experiments_C7_hurdle_heavy.py
...
def main():
    print("=== TRAIN/VAL EXPERIMENTS (Hurdle | No Wrapper | Safe) ===")
                    ...
                    try:
                        ...
                        if algo in ["MLP", "GRU"]:
                            regressor = get_optimized_model(
                                ...
                                X_train=X_train_in, y_train=y_train, light_mode=False, return_cv_results=False
                            )
                            
                            # Construct Hurdle (Torch mode)
                            model = HurdleModel(classifier=tier_gatekeeper, regressor=regressor, regressor_type="torch")
                            
                            # Predict (Gatekeeper uses raw X_val usually, but if it's a pipeline it handles it. 
    
                            
                            # Manual Hurdle Prediction for Evaluation:
                            # 1. Gate Probability (Always use raw X_val, Gatekeeper Pipeline handles scaling if needed)
                            prob_gate = tier_gatekeeper.predict_proba(X_val)[:, 1]
                            
                            # 2. Regressor Prediction (Use X_val_in which is properly scaled for this specific regressor)
                        ...
                        # --- BRANCH 2: STANDARD SKLEARN ---
                            ...
                            # Manual fitting if needed (GLM/HGB usually fit within get_optimized_model but verify)
                        ...
                        rpath = RESULTS_DIR / "train_val_results_C7_Hurdle_heavy.json"
