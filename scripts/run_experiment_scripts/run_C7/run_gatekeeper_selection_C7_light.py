# scripts/run_experiment_scripts/run_C7/run_gatekeeper_selection_C7_light.py
...
CSV_PATH = RESULTS_DIR / "gatekeeper_metrics_light.csv"
...
def main():
    print("=== GATEKEEPER SELECTION (All Tiers | Light Mode) ===")
    ...
    for target in TARGETS:
        ...
        for tier_name in TIER_LIST:
            ...
            n_pos = len(pos_idx)
            if n_pos < 10:
                print(f"SKIPPING {tier_name} | {target}: Too few positives ({n_pos}).")
                del train_df; gc.collect()
                continue

            # Downsample Negatives to 10% (Light Mode usually keeps it light, but this helps balance)
            if neg_mask.sum() > 0:
            ...
            for algo in ALGOS:
                ...
                try:
                    # For LogReg, fit on RAW, but use scaler for optimization input
                    if algo == "LogReg":
                    ...
                    base_model = get_optimized_model(
                        ...
                        input_dim=X_train_in.shape[1], X_train=X_train_in, y_train=y_train_bin, 
                        light_mode=True 
                    )
