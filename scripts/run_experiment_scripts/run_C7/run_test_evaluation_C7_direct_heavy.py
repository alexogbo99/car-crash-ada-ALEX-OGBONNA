...
def load_or_fit_train_scaler(
    ...
    *,
    ...
    - Uses streaming partial_fit via pyarrow when available (no full train load).
    """
    ...
    try:
        ...
        for i, batch in enumerate(pf.iter_batches(columns=feature_cols, batch_size=batch_rows)):
            # Convert batch to numpy, fill NaNs same way as everywhere else
        ...
        # Fallback: full read (can be heavy). Only used if streaming isn't available.
...
def print_model_details(algo, model):
    print(f"   ℹ️  Model Specs for {algo}:")
    try:
        if algo in ["MLP", "GRU"]:
            if algo == "MLP":
...
def main():
    print("=== FINAL TEST EVALUATION (C7 | Direct | Heavy | DUAL METRIC) ===")
    with joblib.parallel_backend('threading', n_jobs=-1):
        
        # HEAVY Direct Script used: "train_val_results.json"
        if not (RESULTS_DIR / "train_val_results.json").exists(): 
            print("No results found."); return
            
        df_res = pd.DataFrame(json.load(open(RESULTS_DIR / "train_val_results.json")))
        
        TARGETS = ["y_7d_sum"]
        TIER_LIST = ["Tier1_Island", "Tier2_Weather", "Tier3_Full"]

        # === THE METRIC LOOP ===
        # We run the selection process twice:
        # 1. Select best models by RMSE
        # 2. Select best models by Poisson Deviance
        
        METRIC_CONFIGS = [
            # (Sort Column in DF, Output Filename)
        ...
        for sort_metric, out_filename in METRIC_CONFIGS:
            ...
            for target in TARGETS:
                ...
                for tier in TIER_LIST:
                    try:
                        # 1. Identify Champion (Sorted by CURRENT METRIC)
                        tier_res = target_res[target_res["tier"] == tier]
                        if tier_res.empty: continue
                        
                        # --- KEY CHANGE: Sort by the current loop's metric ---
                        ...
                        # 3. Load & Apply Scaler (FIT ON TRAIN ONCE, CACHE)
            ...
            # Save the JSON for this specific metric
